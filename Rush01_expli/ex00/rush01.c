...
/*   Updated: 2025/09/07 09:00:41 by bblondin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	run(int *tab, int *views, int size) // lance le solveur et affiche le resultat si resultat il y a 
{
	int	found;

	found = solve(tab, views, size, 0); // on lance la fonction solve
	if (found)
...
int	main(int argc, char **argv) //programme principale argc et argv, argc sa represente le nombre argument passee par commande, argv  
//argc = nombre d argument que l on envoie (nom de l ecxecutable compris)
// argv = une chaine d argument qui contient tout se que l on envoie a la fonction (sa contient nos argument)
{
	...
	if (argc == 2 || argc == 3) // marche aue si 2 ou 3 argument
	{
		if (argc == 3) // si un tableau a une taille particuliere 
			size = read_size(argv[2]);  // on prend la taille en argument
		else
