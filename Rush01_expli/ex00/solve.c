...
/*   Updated: 2025/09/07 08:47:57 by bblondin         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

int	check_row(int *tab, int *views, int size, int i) // gauche a droite puis droite a gauche
{
	int	j;
	...
	while (++j < size) 
	{
		//on prends le premiere element de la liste puis on ajoute j element du tableau qu on veut atteindre si on trouve que maxleft est plus petit est que n_left +1 est aussi plus petit
		if (tab[i / size * size + j] > max_left && ++n_left) // Depuis la gauche : si bâtiment plus haut trouvé, l’ajssigner a max left
			max_left = tab[i / size * size + j]; // on assigne la valeur a max left
		//on prends le premiere element de la liste en partant de la fin
...
int	check_column(int *tab, int *views, int size, int i) // bas en haut
{
	...
	{
		// tab[size * j + i % size)] on va dans le tableau de la colonne est on check si cest plus grand que max top est n_top + 1
		// Depuis le haut : si bâtiment plus haut trouvé, on l’assigne a max_top
		if (tab[size * j + i % size] > max_top && ++n_top) //tab[size * j + i % size] on va dans le tableau tab 
			max_top = tab[size * j + i % size];
		// tab[size * (size - j - 1)] on va dans le tableau de la colonne est on check si cest plus grand que max_bottom est n_bottom + 1
		// Depuis le bas : si bâtiment plus bas trouvé, on l’ajssigne a max_bottom
		//premiere element en partant de la fin
...
int	solve(int *tab, int *views, int size, int i) //i = 0
{
	...
	while (++n <= size)
	{
		j = -1;
		while (++j < size)
			if (tab[i / size * size + j] == n || tab[size * j + i % size] == n) // check si n est egal a une colonne ou ligne
